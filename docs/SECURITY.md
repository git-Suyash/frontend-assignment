...
| Cross-tab cart injection | `StorageEvent` listener in `CartProvider`: detects external writes to `checkout_cart` key; sets `status: 'conflict'` | `src/contexts/CartProvider.tsx` → `handleStorage()` | Warning toast + amber conflict banner in CartPage. Checkout blocked until resolved. |
